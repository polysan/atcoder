...
public class Main {
  public static void main(String[] args) {
    try (Scanner scan = new Scanner(System.in)) {
      ...
      List<Player> filersPlayers = players.stream().filter(player -> player.getColor() == T)
          .collect(Collectors.toList());

      if (filersPlayers.size() < 1) {
        int oneColor = players.get(0).getColor();
        filersPlayers = players.stream().filter(player -> player.getColor() == oneColor).collect(Collectors.toList());
      }
